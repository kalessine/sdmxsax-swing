 ...
 */
public class SeriesSparkline extends Adapter {

    public SeriesSparkline() {
    }
    ...
    public Dataset createDataset(Bindings bindings, List<FullKey> table) {
        ...
        while (it.hasNext()) {
            ...
            ItemType seriesItem = (ItemType) key.getComponent(bindings.getBoundToSeries().getConcept());
            Locale loc = Locale.getDefault();
            TextType ser = seriesItem.findName(loc.getLanguage());
            if (ser == null) {
                ser = seriesItem.findDescription(loc.getLanguage());
            }
            String series = "";
            if (ser != null) {
                series = ser.getText();
            } else {
                series = seriesItem.getId().toString();
            }
            dcd.addValue(value, series, time);
        }
        return dcd;
    }
    ...
    public boolean canCreateDatasetFromBindings(Bindings bindings) {
        ...
        return multiBinds == 0 && time == 1 && continuousBinds == 1&&series==1;
    }
